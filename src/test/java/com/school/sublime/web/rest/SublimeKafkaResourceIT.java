...
import com.school.sublime.config.KafkaProperties;
...
class SublimeKafkaResourceIT {
    ...
    void setup() {
        ...
        SublimeKafkaResource kafkaResource = new SublimeKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/sublime-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/sublime-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
